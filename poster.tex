...
\begin{columns}
  ...
  \block{\huge Approximate Query Processing for SPARQL Endpoints}{
    \begin{multicols}{2}
      ...
      \hl{Sampling-based approximate query processing (S-AQP)} [2] has many
      ...
      S-AQP drastically
      \begin{itemize}
      \item \hl{reduces} execution time, 
      \item delivering \hl{approximate results}
      \item with \hl{error} estimates.
      \end{itemize}
    \end{multicols}%
  }
  %  \VRULE
  ...
  \block{Ad-hoc sampling}{
    \begin{center}
      ...
      \uppercase{May \hl{time out} on public SPARQL endpoints\ldots}
      \HRULE
    \end{center}
...
\block{Random walks for cardinality estimates and confidence with Wander~Join~[2]}{
  ...
  \fbox{
    \begin{tikzpicture}
      ...
      \draw (0, 0) node(A){\hl{\textbf{A}}} +(-\RECTSIZE, -\RECTSIZE) rectangle +(\RECTSIZE, \RECTSIZE);
      \draw (\X, 0) node(D){\hl{\textbf{D}}} +(-\RECTSIZE, -\RECTSIZE) rectangle +(\RECTSIZE, \RECTSIZE);
      ...
      \draw (2.2*\X, \Y) node(Q27611){\hl{\textbf{Q27611}}} +(-3*\RECTSIZE, -\RECTSIZE) rectangle +(3*\RECTSIZE, \RECTSIZE);
  ...
  %% \adjustbox{max width=0.37\columnwidth}{\textbf{Query $Q_1$: All cycling
  %%   races of Central America along with their respective country.}}
  %% \hspace{7cm}
  %% \adjustbox{max width=0.45\columnwidth}{\textbf{RDF Graph $G_1$: $A$, $B$,
  %%   and $C$ are cycling sports; $D$ and $E$ are countries of Central
  %%   America.}}
}
...
\begin{columns}
  ...
  \block{}{
    ...
    \begin{center}
      ...
      \hl{$P(\gamma_i) = |\left\llbracket tp_1 \right\rrbracket_G|^{-1} \prod_{i=2}^{n}
    ...
    \begin{small}
    \begin{tabular}{lll}
      $P(\gamma_1)^{-1} =|\left\llbracket (?x1, P641, Q3609) \right\rrbracket_{G_1}| \cdot$
       $|\left\llbracket (\textbf{A}, P17, ?x3) \right\rrbracket_{G_1}| \cdot$ 
       $|\left\llbracket (\textbf{D}, P361, Q27611) \right\rrbracket_{G_1}|$
       $= 2 \cdot 1 \cdot 1$
    \end{tabular}
    \end{small}
  }
  ...
  \block{Complexity}{
    ...
    \item retrieving random triples from triple patterns.
    \end{inparaenum}
      
    Common SPARQL query engines -- such as Apache Jena, Virtuoso,
    RDF4J, Blazegraph -- rely on \hl{balanced trees}.\\

    We achieve a \hl{logarithmic} upper-bound on time complexity !
  }
  \VRULELEFT
\end{columns}
...
\begin{columns} 
  ...
  \block{Conclusion and Future Work}{
    \hl{RAW-JENA provides a \textbf{pay-as-you-go} S-AQP by implementing random walks on top of
      Apache Jena.} It does not need to reingest the data, everything is
    already here, and it was quite easy to implement.
    The code is open source and available at \dotfill >

    \vspace{0.75em}
    
    However, we need \hl{more} than conjunctive queries, and
    \hl{more} SPARQL engines with such a feature. We need a \hl{standard}!
  }
  \block{}{
    \small
    \vspace{0.55em}
