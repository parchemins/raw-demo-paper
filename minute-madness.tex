...
%%% processing summaries which is impossible as of today, embeddings, and exploratory queries.
...
\begin{frame}{
    \vspace{-0.5em}
    \begin{tikzpicture}
      \node(global_title)[anchor=west, align=left, style={scale=0.86}] at (0,1pt) {RAW-JENA: Approximate Query Processing for SPARQL Endpoints};
  ...
  \begin{tikzpicture}[scale=0.35, every node/.style={scale=0.9}]
    ...
    \draw (sparql_query.south west) -- (sparql_query.south) -- (thinker.north east) -- (sparql_query.south east);
