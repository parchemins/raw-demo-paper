...
method times out for $r$ above $100M$. Sampling methods relying on
\texttt{ORDER BY RAND()} also time out~\REF. Ad-hoc methods enabled by
some SPARQL endpoints allow for more efficient random draws from
triple
...
underlying complexity remains unspecified\todo{or ``is not
  established'' was good too}.  On SPARQL, there is nothing equivalent
to the \texttt{TABLESAMPLE} clause that is part of the SQL standard
since 2003.
...
In this demonstration, we extended Apache Jena to support efficient
sampling-based approximate query processing (S-AQP) for
\TODO{core}\todo{maybe keep ``conjunctive'' only. in abstract too}
conjunctive SPARQL queries. This extension, called \NAME, delivers
random results of the given query $Q$ with a logarithmic time
complexity: $\mathcal{O}(k.log(|G|)$ where $k$ is the number of
triple/quad patterns of $Q$, and $|G|$ is the dataset size. \NAME also
provides the cardinality estimation and confidence interval of
$Q$ in a pay-as-you-go fashion~\cite{li2019wanderjoin}: sampling a
query multiple times improves on the accuracy of provided estimates.
